...
type Student = { id: string; name: string; registration_number: string; department: string; academic_year: number; section: string; bus_code: string | null; route_name: string | null; driver_name: string | null; bus_status: string | null; last_seen_at: string | null; relationship: string };
...
type PortalData = { students: Student[]; attendance: Attendance[]; scans: Scan[]; timetable: Timetable[] };
type Tab = 'overview' | 'attendance' | 'journey' | 'timetable';
const nav = [{ id: 'overview', label: 'Overview', icon: LayoutDashboard }, { id: 'attendance', label: 'Attendance', icon: Fingerprint }, { id: 'journey', label: 'Bus journey', icon: Radar }, { id: 'timetable', label: 'Timetable', icon: CalendarDays }] as const;
...
export default function Portal({ session, onLogout }: { session: Session; onLogout: () => void }) {
  ...
  useEffect(() => {
    ...
    const poll = window.setInterval(() => void refresh(true), 60_000);
  ...
  return <div className="dashboard-shell portal-shell">
    <aside className={`sidebar ${mobileNav ? 'sidebar-open' : ''}`}>
      ...
      <nav className="side-nav"><span className="side-label">MY CAMPUS</span>{nav.map(({ id, label, icon: Icon }) => <button key={id} className={tab === id ? 'active' : ''} onClick={() => { setTab(id); setMobileNav(false); }}><Icon size={18}/><span>{label}</span></button>)}</nav>
    ...
    <main className="dashboard-main">
      ...
      <div className="dashboard-content portal-content">
        ...
        {data && !data.students.length && <div className="empty-panel"><GraduationCap size={36}/><h3>No student linked yet</h3><p>Ask your campus administrator to link your account to a student registration number.</p></div>}
        {student && <>
          {data!.students.length > 1 && <div className="portal-selector"><span>Viewing student</span><select value={selected} onChange={(event) => setSelected(event.target.value)}>{data!.students.map((item) => <option value={item.id} key={item.id}>{item.name} · {item.registration_number}</option>)}</select></div>}
          {tab === 'overview' && <motion.div className="page-stack" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="portal-stat-grid"><div className="portal-stat"><CheckCircle2/><span>Confirmed present</span><strong>{confirmed}</strong><small>of {reviewed} reviewed sessions</small></div><div className="portal-stat"><Clock3/><span>Awaiting review</span><strong>{records.filter((item) => item.status === 'PROVISIONAL').length}</strong><small>Faculty confirmation pending</small></div><div className="portal-stat"><BusFront/><span>Assigned bus</span><strong>{student.bus_code || '—'}</strong><small>{student.route_name || 'No bus assigned'}</small></div><div className="portal-stat"><CalendarDays/><span>Weekly classes</span><strong>{classes.length}</strong><small>From the live timetable</small></div></div>
