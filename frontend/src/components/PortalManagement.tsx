...
export default function PortalManagement({ token, refreshKey, onCreate, onLink }: { token: string; refreshKey: number; onCreate: (role: 'STUDENT' | 'PARENT') => void; onLink: () => void }) {
  ...
  return <div className="page-stack portal-management">
    <div className="page-title-row"><div><span className="section-kicker">ROLE ACCESS</span><h2>Student & parent portals</h2><p>Students can register with a real bus and RFID card. Parents can register against contact details on a child's record. You can also create and link accounts here.</p></div></div>
    ...
    <div className="portal-management-flow"><span>1 · Register a bus</span><ArrowRight size={16}/><span>2 · Student registers with card and bus</span><ArrowRight size={16}/><span>3 · Parent links with recorded contact</span></div>
