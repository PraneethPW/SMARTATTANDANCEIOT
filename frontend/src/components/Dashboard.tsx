...
export default function Dashboard({ session, onLogout }: { session: Session; onLogout: () => void }) {
  ...
  return (
    <div className="dashboard-shell">
      ...
      {panel === 'user' ? <UserPanel token={session.token} onClose={() => setPanel(null)} onCreated={() => { setPanel(null); toast('Team account created'); }} /> : null}
...
function UserPanel({token,onClose,onCreated}:{token:string;onClose:()=>void;onCreated:()=>void}){const [error,setError]=useState('');const [busy,setBusy]=useState(false);const submit=async(e:FormEvent<HTMLFormElement>)=>{e.preventDefault();setBusy(true);setError('');const d=new FormData(e.currentTarget);try{await api('/api/users',{method:'POST',body:JSON.stringify({name:d.get('name'),email:d.get('email'),password:d.get('password'),role:d.get('role')})},token);onCreated();}catch(err){setError(err instanceof Error?err.message:'Could not create account');}finally{setBusy(false);}};return <Panel title="Add team account" subtitle="Create a role-scoped faculty, transport, or administrator login." onClose={onClose}><FormShell onSubmit={submit} error={error} busy={busy} button="Create secure account"><label>Full name<input name="name" required minLength={2}/></label><label>Email address<input name="email" type="email" required/></label><label>Temporary password<input name="password" type="password" required minLength={10}/></label><label>Role<select name="role"><option value="FACULTY">Faculty</option><option value="TRANSPORT">Transport operator</option><option value="ADMIN">Administrator</option></select></label></FormShell></Panel>}
